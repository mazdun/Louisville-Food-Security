...
* In Louisville and its peer cities, the childhood food insecurity rate is much worse than the overall food insecurity rate. The COVID-19 pandemic further increased the share of children experiencing food insecurity. 
...
* Louisville needs about $46 million annually to meet its current food needs. Its peers need about $51 million, on average. 
...
Here's a closer look at food access in Louisville. It includes the percentage of the population that is considered low-income and far from a grocery store, the percentage that does not have a vehicle and is far from a grocery store and the percentage that relies on certain food benefits and is far from a grocery store.

## Limited Access to Grocery Stores {.tabset}
...
* **How Louisville's Currently Doing:** Louisville currently ranks 7th on this metric, with about 6% of residents living with low incomes and far from grocery stores. That's about half the rate of Louisville's currently worst performing peer city, Birmingham. 
* **How Louisville's Performed Over Time:** Louisville has seen its numbers getting worse over the past five years of most recent data. Generally, over the same time period, Louisville's peers have been improving. 
...
### Ranking
*In rural communities, far from a grocery store means more than 10 miles; in non-rural areas, it's more than 1 mile. The race data considers the entire population (not just those with low incomes) who live more than 10 miles from a grocery store.*
```{r food access ranking}
load("intermediate_data/food_access_county.rda")

this_title <- "Limited Access to Grocery Stores"
...
make_map(kentucky_food_atlas_data, 'lalowi1share', 'Low Access and Low Income')
```

## Limited Access to Grocery Stores and Vehicle Access {.tabset}

It becomes significantly more challenging to access nutritious food at a grocery store if a household lives far away and does not have access to a vehicle. These households often must rely on public transportation or shop at stores that are closer to them that also potentially have less nutritious food. 

* **How Louisville's Currently Doing:** More than 2% of Louisville's households live more than a mile from a grocery store and do not have a vehicle. That puts it in the bottom half of its peer cohort. 
* **How Louisville's Performed Over Time:** Over the last five years of most recent data, Louisville has more than doubled this rate. That makes it the worst performer of its peers. 
...
make_map(kentucky_food_atlas_data, 'lahunv1share', 'Low Access Households without Vehicle')
```
...
* **How Louisville's Currently Doing:** 2.6% of Louisville's households receive SNAP benefits and live more than a mile from a grocery store. That puts the city in the top half of its peers.  
* **How Louisville's Performed Over Time:** Louisville's rate has increased (though modestly) over the past five years in which data is available. However, its peer cities have seen their rates decrease. That makes Louisville the worst performer of its peers.
...
make_map(kentucky_food_atlas_data, 'lasnap1share', 'Low Access and SNAP')
```

# Food Environment

What does the food environment look like in Louisville? Here's a breakdown of the number of grocery stores, convenience stores, fast food restaurants and farmers markets per 100,000 people.
...
The number of grocery stores, convenience stores and fast food restaurants has remained relatively constant since 2009. Even so, Louisville currently has more grocery stores per 100,000 people than its peer cities, on average. It also has significantly fewer convenience stores than its peers, on average. 
...
Here's an estimate from Feeding America and Nielsen of how much money (annually) is needed in Louisville and its peer cities to address food insecurity. 

The budget shortfall has decreased over time in Louisville and its peer cities. (It's important to note, however, that the percentage of people experiencing food insecurity has also decreased over this time.) 

Toward the beginning of the last decade, Louisville had a larger budget shortfall than its peers, on average. Since that point, Louisville has diverged from its peers. Currently, the food insecure in Louisville need about \$46 million collectively to meet their food needs. That's lower than the peer average of about \$51 million.

```{r budget shortfall, fig.width=36}
